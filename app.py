...
def _normalize_phone(raw: str) -> str:
    raw = (raw or "").strip().replace("whatsapp:", "").strip()
...
def _ensure_lead(ws_leads, from_phone: str, msg_in: str):
    ...
    records = get_records_cached(ws_leads, cache_seconds=2)
    ...
    # Detecta fuente por primer mensaje
    fuente = "DESCONOCIDA"
    t = (msg_in or "").lower()
...
def whatsapp_webhook():
    ...
    try:
        ...
        # Si está en INICIO y el usuario manda "Hola", NO lo valides; solo muestra INICIO
        if estatus == "INICIO":
            opt_try = _normalize_option(msg_in)
            if opt_try not in ("1", "2"):
                out = str((cfg.get("INICIO") or {}).get("Texto_Bot") or "").strip()
                if not out:
                    out = ("Hola, soy Ximena 👋\n\n"
                           "Te acompaño para orientarte y asignarte un abogado.\n\n"
                           "¿Deseas continuar?\n1 Sí\n2 No")
